...
"""Dataset tools utilities for LeRobotDatasetV3.
...
def delete_episodes(
    ...
    """Delete episodes from a LeRobotDatasetV3 and create a new dataset.

    Args:
        dataset: The source LeRobotDatasetV3.
    ...
    new_meta = LeRobotDatasetMetadataV3.create(
        repo_id=repo_id,
...
def split_dataset(
    ...
    output_dir: str | Path | None = None,
) -> dict[str, LeRobotDatasetV3]:
    """Split a LeRobotDatasetV3 into multiple smaller datasets.

    Args:
        dataset: The source LeRobotDatasetV3 to split.
...
def merge_datasets(
    datasets: list[LeRobotDatasetV3],
    output_repo_id: str,
    output_dir: str | Path | None = None,
) -> LeRobotDatasetV3:
    """Merge multiple LeRobotDatasetV3s into a single dataset.

    This is a wrapper around the aggregate_datasets functionality with a cleaner API.

    Args:
        datasets: List of LeRobotDatasetV3s to merge.
    ...
    merged_dataset = LeRobotDatasetV3(
...
def modify_features(
    ...
    """Modify a LeRobotDatasetV3 by adding and/or removing features in a single pass.
...
def add_features(
    ...
    """Add multiple features to a LeRobotDatasetV3 in a single pass.
...
def remove_feature(
    ...
    """Remove features from a LeRobotDatasetV3.
...
def _copy_and_reindex_data(
    src_dataset: LeRobotDatasetV3,
    dst_meta: LeRobotDatasetMetadataV3,
...
def _write_parquet(df: pd.DataFrame, path: Path, meta: LeRobotDatasetMetadataV3) -> None:
...
def _copy_data_with_feature_changes(
    dataset: LeRobotDatasetV3,
    new_meta: LeRobotDatasetMetadataV3,
