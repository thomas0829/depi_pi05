...
class PI05Pytorch(nn.Module):  # see openpi `PI0Pytorch`
    ...
    def __init__(self, config: PI05Config, rtc_processor: "RTCProcessor | None" = None):
        ...
        logging.info("PI05Pytorch model initialized successfully")
...
class PI05Policy(PreTrainedPolicy):
    ...
    def merge_lora_weights(self) -> None:
        ...
        This permanently merges LoRA adapters into the base model weights.
        After calling this, the model can be saved and loaded without LoRA dependencies.
        
        Warning: This operation is irreversible. The model cannot be further trained with LoRA
        after merging. Only call this when you want to export the final model for inference.
        """
        has_lora = hasattr(self.model.paligemma_with_expert.paligemma.language_model, 'merge_and_unload')
        
        if not has_lora:
            logging.warning("No LoRA adapters detected. Nothing to merge.")
            return
        
        logging.info("Merging LoRA adapters into base model...")
        
        # Merge LoRA weights back into the base model
        self.model.paligemma_with_expert.paligemma.language_model = \
            self.model.paligemma_with_expert.paligemma.language_model.merge_and_unload()
        
        self.model.paligemma_with_expert.gemma_expert.model = \
            self.model.paligemma_with_expert.gemma_expert.model.merge_and_unload()
        
        # Update config to reflect that LoRA is no longer active
        self.config.use_lora = False
        
        logging.info("✓ LoRA weights merged successfully - model is now LoRA-free")
    ...
    def init_rtc_processor(self):
        ...
        if self.config.rtc_config is not None:
            try:
                ...
                logging.warning("RTC module not available. Disabling RTC functionality.")
